...
os.environ['PATH'] = os.environ.get('PATH', '') + os.pathsep + 'C:\\Program Files\\Graphviz\\bin'
...
def generateTransitionTable(finite_automata):
    # Create a PyDot Graph object
    graph = Dot(format='png', graph_type='digraph', prog=r'C:/Program Files/Graphviz/bin/dot.exe')

    def addStateTransitions(state, states_done, symbol_table):
        if state in states_done:
            return
        ...
        for symbol in state.next_state:
            for ns in state.next_state[symbol]:
                ...
                # Add nodes and edges
                node_from = Node(symbol_table[state])
                node_to = Node(symbol_table[ns])
                edge = Edge(node_from, node_to, label=symbol)
                graph.add_node(node_from)
                graph.add_node(node_to)
                graph.add_edge(edge)

                addStateTransitions(ns, states_done, symbol_table)

    # Start from the initial state
    node_initial = Node("q0", shape="doublecircle")  # Initial state
    graph.add_node(node_initial)
    addStateTransitions(finite_automata[0], [], {finite_automata[0]: "q0"})
